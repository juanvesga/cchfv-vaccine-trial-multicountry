...
# This script runs an MCMC algorithm (fitR) using South Africa specific data
...
########################################################################################
country <- "SA"
driver <- "sat_def" # soil_t, sat_def, rel_hum, NDVI
...
my_prior <- function(theta) {
  ## uniform prior on A
  log.prior.A <- dunif(theta[["A"]], min = 0.1, max = 10, log = TRUE)
  ...
  log.prior.F_risk <- dunif(theta[["F_risk"]], min = 5e-2 * 1e3, max = 10e-1 * 1e3, log = TRUE) # 0.01 / 5
...
my_posterior <- function(theta, label = names) {
  ...
  loglik_liv_prev_age <- sum(dbinom(
    ...
    prob = sim.sir$l_prev_age3g, log = TRUE
  ...
  loglik_prev_farmer <- sum(dbinom(
    ...
    prob = sim$h_prev_farmer, log = FALSE
  ...
  loglik_prev_other <- sum(dbinom(
    ...
    prob = sim$h_prev_other, log = FALSE
