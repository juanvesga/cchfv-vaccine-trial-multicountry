...
sir_model <- odin::odin(
  ...
  # beta temperature dependent
  beta_LX <- envr_beta_vec / (duration * N)

  # Prob of infection & news infections in livestock
  foi <- 1 - exp(-beta_LX * infectious)
...
get_output_sir <- function(params, theta)
  ...
  for (jj in 1:nruns) {
    ...
    params$theta <- c(
      A = theta$A[jj],
      D_inf_L = theta$D_inf_L[jj],
      D_lact_liv = theta$D_lact_liv[jj],
      D_imm_liv = theta$D_imm_liv[jj]
    )
    params$recover <- 1 / params$theta[["D_inf_L"]]
    params$time_passimm_loss_livestock <- 1 / params$theta[["D_lact_liv"]] # rate of moving from passive immnity at birth to susceptible
    params$time_susceptible_livestock <- 1 / params$theta[["D_imm_liv"]] # rate of moving from passive immnity at birth to susceptible
    ...
    simu_age_R <- c(
      mean(out$R1[(field_work_startL:field_work_endL)] + out$Ri[(field_work_startL:field_work_endL)]),
      mean(out$R2[(field_work_startL:field_work_endL)]),
      mean(out$R3[(field_work_startL:field_work_endL)]),
      mean(out$R4[(field_work_startL:field_work_endL)]),
      mean(out$R5[(field_work_startL:field_work_endL)])
    )

    simu_age_N <- c(
      mean(L_1[(field_work_startL:field_work_endL)]),
      mean(L_2[(field_work_startL:field_work_endL)]),
      mean(L_3[(field_work_startL:field_work_endL)]),
      mean(L_4[(field_work_startL:field_work_endL)]),
      mean(L_5[(field_work_startL:field_work_endL)])
    )

    simu_age_R_peak <- c(
      mean(out$R1[(field_work_startL + 1)] + out$Ri[(field_work_startL + 1)]),
      mean(out$R2[(field_work_startL + 1)]),
      mean(out$R3[(field_work_startL + 1)]),
      mean(out$R4[(field_work_startL + 1)]),
      mean(out$R5[(field_work_startL + 1)])
    )

    simu_age_N_peak <- c(
      mean(L_1[(field_work_startL + 1)]),
      mean(L_2[(field_work_startL + 1)]),
      mean(L_3[(field_work_startL + 1)]),
      mean(L_4[(field_work_startL + 1)]),
      mean(L_5[(field_work_startL + 1)])
    )
    ...
    l_prev_age4g[jj, ] <- c(
      simu_age_R[1], simu_age_R[2],
      simu_age_R[3], simu_age_R[4] +
        simu_age_R[5]
    ) /
      c(
        simu_age_N[1], simu_age_N[2],
        simu_age_N[3], simu_age_N[4] +
          simu_age_N[5]
      )
    ...
    simu_all_R <- c(out$R1 + out$R2 + out$R3 + out$R4 + out$R5)
    l_prev_all_long[jj, ] <- simu_all_R / simu_all_N_long

    simu_all_R <- c(
      mean(simu_all_R[(field_work_startLall[1]:field_work_endLall[1])]),
      mean(simu_all_R[(field_work_startLall[2]:field_work_endLall[2])])
    )

    simu_all_N <- c(
      mean(simu_all_N_long[(field_work_startLall[1]:field_work_endLall[1])]),
      mean(simu_all_N_long[(field_work_startLall[2]:field_work_endLall[2])])
    )
...
# Call model function and organize/format model outputs
get_output_seir <- function(params, theta, n.replicates = 1) 
  {
  nruns <- nrow(theta)
  mo_length <- params$nt
  yr_length <- length(unique(as.Date(temp_month$month, format = "%Y")))
  ...
  # Allocate memory
  h_incidence_reported_month <- matrix(NA, nrow = nruns, ncol = mo_length)
  h_incidence_month <- matrix(NA, nrow = nruns, ncol = mo_length)
  h_incidence_clinical_month <- matrix(NA, nrow = nruns, ncol = mo_length) # new
  h_incidence_reported_year <- matrix(NA, nrow = nruns, ncol = yr_length)
  h_fatality_month <- matrix(NA, nrow = nruns, ncol = mo_length)
  h_incidence_farmer <- matrix(NA, nrow = nruns, ncol = mo_length)
  h_incidence_other <- matrix(NA, nrow = nruns, ncol = mo_length)
  h_fatality_year <- matrix(NA, nrow = nruns, ncol = yr_length) # new
  h_prev_farmer <- matrix(NA, nrow = nruns, ncol = length(field_work_endH))
  h_prev_other <- matrix(NA, nrow = nruns, ncol = length(field_work_endH))
  ...
  init.seir <- c(
    F_S = params$N_F - round(params$N_F * params$prop_F_R),
    F_V = 0,
    F_Vp = 0,
    F_E = 0,
    F_I = 0,
    F_R = round(params$N_F * params$prop_F_R),
    O_S = params$N_O - round(params$N_O * params$prop_O_R),
    O_V = 0,
    O_Vp = 0,
    O_E = 0,
    O_I = 0,
    O_R = round(params$N_O * params$prop_O_R)
  )
  ...
  for (jj in 1:nruns) {
    ...
    t <- seq(1, params$nt)
    out <- as.data.frame(run_seir(init.seir, params, t, n.replicates))

    #------------------------------------ 
    # Process and format model output
    #------------------------------------ 

    dates <- temp_month$month[1:params$nt]
    tmp <- data.frame(
      date = dates,
      cases_tmp = out$F_incidence + out$O_incidence,
      deaths = out$F_fatality + out$O_fatality
    )

    tmp$cases <- ifelse(tmp$cases_tmp == 0, 1e-6, as.numeric(paste(tmp$cases_tmp)))
    tmp$cases_tmp <- NULL
    tmp$reported_cases_mo <- tmp$cases * params$theta[["RRreport"]] * params$theta[["clin_frac"]]
    tmp$reported_deaths_mo <- tmp$deaths * params$theta[["RRreport"]]
    #
    human_inc_yr <- tmp %>%
      mutate(year = as.Date(date, format = "%Y")) %>%
      group_by(year) %>%
      summarise(reported_yr = sum(reported_cases_mo)) %>%
      mutate(reported_cases_yr = ifelse(reported_yr == 0, NA, as.numeric(paste(reported_yr))))

    human_death_yr <- tmp %>%
      mutate(year = as.Date(date, format = "%Y")) %>%
      group_by(year) %>%
      summarise(fatalities_yr = sum(reported_deaths_mo))


    h_incidence_reported_month[jj, ] <- tmp$reported_cases_mo
    h_incidence_month[jj, ] <- tmp$cases
    h_incidence_clinical_month[jj, ] <- tmp$cases * params$theta[["clin_frac"]]
    h_incidence_reported_year[jj, ] <- human_inc_yr$reported_cases_yr
    h_fatality_month[jj, ] <- out$F_fatality + out$O_fatality
    h_incidence_farmer[jj, ] <- out$F_incidence
    h_incidence_other[jj, ] <- out$O_incidence
    h_fatality_year[jj, ] <- human_death_yr$fatalities_yr

    # Human prevalence (farmers and others)


    for (yy in 1:length(field_work_endH)) {
      simu_F_R <- mean(out$F_R[(field_work_startH[yy]:field_work_endH[yy])])
      simu_F_N <- c(out$F_S + out$F_E + out$F_I + out$F_R)
      simu_F_N <- mean(simu_F_N[(field_work_startH[yy]:field_work_endH[yy])])
      h_prev_farmer[jj, yy] <- simu_F_R / simu_F_N


      simu_O_R <- mean(out$O_R[(field_work_startH[yy]:field_work_endH[yy])])
      simu_O_N <- c(out$O_S + out$O_E + out$O_I + out$O_R)
      simu_O_N <- mean(simu_O_N[(field_work_startH[yy]:field_work_endH[yy])])
      h_prev_other[jj, yy] <- simu_O_R / simu_O_N
    }
  ...
  sim <- list(
    h_incidence_reported_year = h_incidence_reported_year,
    h_fatality_month = h_fatality_month,
    h_fatality_year = h_fatality_year,
    h_incidence_reported_month = h_incidence_reported_month,
    h_incidence_month = h_incidence_month,
    h_incidence_clinical_month = h_incidence_clinical_month,
    h_incidence_farmer = h_incidence_farmer,
    h_incidence_other = h_incidence_other,
