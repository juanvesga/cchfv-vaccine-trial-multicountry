...
sir_model <- odin::odin({
  ...
  dim(params) <- user()
  dim(seed_t) <- user()
  dim(newvacc_y) <- user() # c(length(newVacc),length(envirom_beta_t))
  dim(seed) <- 5
  dim(mu) <- 5
  dim(ageing) <- 5
})
...
seir_stoc <- odin::odin(
  {
    ...
    # Risk of infection vector (from livestock SIR)
    risk_livestock <- interpolate(risk_livestock_t, risk_livestock_y, "linear")

    # Vaccination schedule vector
...
# Wrapping function to get parameters , organize input, create model and run
...
# Call SIR model function and organize/format model outputs
get_output_sir <- function(params, theta) {
  ...
  l_prev_age3g <- matrix(NA, nrow = nruns, ncol = 3)
...
# Wrapping function to get parameters , organize input, create model and run SEIR
...
get_output_seir <- function(params, theta, n.replicates = 1) {
  ...
  for (jj in 1:nruns) {
    ...
    #                report_slope=theta$report_slope[jj])
